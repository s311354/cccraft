...
// 
// Most function in this file are named after the symbols they are
...
struct Initializer {
    ...
    // `chidren` has initializers for its children.
...
// Likewise, global variables are accumlated to this list.
static Obj *globals;
...
// That can also be writen as `static long` because you can omit
...
static Type *declspec(Token **rest, Token *tok, VarAttr *attr) {
    // We use a single integer as counters for all typename.
...
// array-initializer = "{" initializer ("," initilaizer)* "}"
...
static bool is_typename(Token *tok) {
    static char *kw[] = { "void", "_Bool", "char", "short", "int", "long", "struct", "union", "typedef", "enum", "static", };
...
static Node *new_inc_dec(Node *node, Token *tok, int addend) {
    ...
    return new_cast(
               new_add(
                       to_assign(new_add(node, new_num(addend, tok), tok)),
                       new_num(-addend, tok),
                       tok),
               node->ty);
}
...
static Node *funcall(Token **rest, Token *tok) {
    ...
    if (!sc)
        error_tok(start, "implicit declaration on a function");
...
// pargram = (typedef | function-definition | global-variable)*
Obj *parse(Token *tok) {
    globals= NULL;
